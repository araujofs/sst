...
**Ambiente de Cálculo**: Utilizado por contadores e gerentes em escritórios de contabilidade, departamentos financeiros de empresas e consultorias. Os profissionais acessam o sistema através de navegadores web modernos (Chrome, Firefox, Edge ou Safari) em desktops, notebooks, tablets ou celulares. O acesso ocorre remotamente via internet, permitindo trabalho em home office ou durante atendimento a clientes. Neste ambiente, são realizadas as principais operações do sistema: cálculos tributários, simulações de cenários, comparação entre regimes fiscais, geração de relatórios e análises estratégicas. A autenticação é obrigatória e cada usuário possui permissões específicas de acordo com seu perfil profissional.

**Ambiente do Cliente**: Utilizado pelos clientes que solicitam serviços aos escritórios de contabilidade para visualizar dashboards com gráficos, insights e relatórios gerados durante o uso do sistema pelos contadores. De maneira que esses clientes possam acompanhar a situação atual e ter menos demora no feedback.
...
O Sistema de Simulação Tributária (SST) é uma plataforma web especializada com o intuito de facilitar os cálculos e as simulações de impostos, permitindo que profissionais da área da contabilidade e financeira realizem análises fiscais com maior precisão e agilidade para demonstrações de resultado para os seus clientes.
...
Por ser totalmente web, o SST deve ser acessado online mediante assinatura digital ou compra da licença. Ele não exige nenhum hardware específico muito potente e pode ser usado em navegadores como Chrome, Firefox, Safari e Edge, tanto em computadores quanto em dispositivos móveis para os usuários.

Portanto, o sistema SST pode se sincronizar a recursos externos, como APIs de legislação e bancos de dados com informações fiscais, tornando possível a notificação automática de atualizações de alíquotas e normas tributárias para os usuários.
...
O produto SST será disponibilizado para os usuários no modelo de assinatura, podendo ser mensal ou anual, dependendo do pacote que o cliente preferir assinar, o qual vai garantir acesso remoto e contínuo ao sistema via navegador web. Os usuários não terão direito de adquirir o software via instalação local, mas sim o direito de uso temporário de dispositivo via web, confome o plano contratado.

Alternativamente, o sistema poderá ser oferecido sob modelo de licença mediante pagamento único, contratando nossa manutenção especializada sob demanda e pagamento recorrente. A sua instalação e manutenção serão realizadas de forma regular e integralmente pela equipe provedora, conforme demanda do usuário (e pagamento), incluindo hospedagem em servidores, atualizações automáticas do sistema e suporte técnico remoto sem disponibilização do código-fonte.
